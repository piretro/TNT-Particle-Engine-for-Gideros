-- ############################################# --
-- ## TNT Particles Engine v1.16              ## --
-- ## By Gianluca D'Angelo Copyright (C) 2012 ## --
-- ############################################# --
-- official website with full documentation http://www.tntparticlesengine.com/

-- Version 1.16  release note
-- update to use gideros-lua math operators  (sinistersoft)

-- Version 1.15  release note
-- update to use the new Particles sprite  (sinistersoft)
-- Particle textures need to have same width&height
			...
			for j = 1, self.particlesCounter do
				...
				for j = 1, particles.particlesMax do
					...
					if particlePoint.isAlive then
						particles.particlesGroup:setParticleColor(particlePoint.sprite,0,0)
					end
		...
		for j = 1, particles.particlesMax do
			...
			--			if particlePoint.sprite:isVisible() then
				...
				--		end
					...
					if cSize ~= size then
						particles.particlesGroup:setParticleSize(particlePoint.sprite,cSize*particles.textureSize)
					end
					----------------------------------------------------------
					-- ALPHA MORPH IN & OUT-----------------------------------
					----------------------------------------------------------
					...
					if particles.rotation then
						...
						particles.particlesGroup:setParticleAngle(particlePoint.sprite,-particlePoint.rotation)
					else
						local newAngle = qDeg(self.emitter.direction + particlePoint.direction)
						if particlePoint.crotation ~= newAngle then
							particles.particlesGroup:setParticleAngle(particlePoint.sprite,-newAngle)
					...
					-- WRITE SPRITE ----------------------------------------
					--------------------------------------------------------
					local colour=(((255*currentR)&0xff)<<16)|(((255*currentG)&0xff)<<8)|((255*currentB)&0xff)
					particles.particlesGroup:setParticleColor(particlePoint.sprite,colour,alpha)
					particles.particlesGroup:setParticlePosition(particlePoint.sprite,particlePoint.moveXPos + particlePoint.xPos + particlePoint.xDisplacement + particles.xOffset, particlePoint.moveYPos + particlePoint.yPos + particlePoint.yDisplacement + particles.yOffset)
				end
				...
				if (timePassed > particlePoint.bornDelay) then
					...
					if (particlePoint.loopCounter <= particles.loopCounter) then
						if self.emitter.start then
							...
							-- Re-Bird Particles ----------------------------------------------------
							-------------------------------------------------------------------------
								if not self.emitter.hide then
									local colour=(((255*particlePoint.r)&0xff)<<16)|(((255*particlePoint.g)&0xff)<<8)|((255*particlePoint.b)&0xff)
									--particles.particlesGroup:setParticleColor(particlePoint.sprite,colour,1)
								end
								particles.particlesGroup:setParticlePosition(particlePoint.sprite,particlePoint.moveXPos + particlePoint.xPos + particlePoint.xDisplacement + particles.xOffset, particlePoint.moveYPos + particlePoint.yPos + particlePoint.yDisplacement + particles.yOffset)
								particles.particlesGroup:setParticleAngle(particlePoint.sprite,-qDeg(self.emitter.direction + particlePoint.direction))
								if particlePoint.currentSize ~= 1 then
									particles.particlesGroup:setParticleSize(particlePoint.sprite,particlePoint.size*particles.textureSize)
								end
								particlePoint.lifeStart = os.timer()
...
function CParticles:init(particlesSprite, particlesMax, particlesMaxLife, particlesMaxStartupDelay, blendingMode, particleFixedStartupDelay)
	...
	self.particles = {
		...
		particlesGroup = Particles.new(), -- particles sprite group
		textureSize=1,
	...
	if particlesSprite then
		self.particles.particlesGroup:setTexture(particlesSprite)
		self.particles.textureSize=particlesSprite:getWidth()
	end
	if blendingMode then self.particles.particlesGroup:setBlendMode(blendingMode) end
	...
	for j = 1, self.particles.particlesMax do
		...
		local newParticle = self.particles.particlesGroup:addParticles(0,0,self.particles.textureSize,0,0) -- x,y,size,angle,ttl
		...
		self.particles.particlesList[j].sprite = newParticle
		...
		self.particles.particlesGroup:setParticleColor(self.particles.particlesList[j].sprite,0,0)
	end -- for
end
